...
if [ -d "$KAIROSDB_CONF_DIR/logging" ]; then
    CLASSPATH="$KAIROSDB_CONF_DIR/logging"
elif [ "$KAIROSDB_CONF_DIR/*xml" != "" ]; then
    CLASSPATH="$KAIROSDB_CONF_DIR"
fi
...
if [ "$1" = "run" ] ; then
	...
	exec "$JAVA" $JAVA_OPTS -cp $CLASSPATH org.kairosdb.core.Main \
		-c start -p $KAIROSDB_CONF_FILE >> $KAIROSDB_LOG_FILE 2>&1 &
	echo $! > "$KAIROSDB_PID_FILE"
elif [ "$1" = "stop" ] ; then
	shift
	kill `cat $KAIROSDB_PID_FILE` > /dev/null 2>&1
	while kill -0 `cat $KAIROSDB_PID_FILE` > /dev/null 2>&1; do
		echo -n "."
		sleep 1;
	done
	rm $KAIROSDB_PID_FILE
elif [ "$1" = "export" ] ; then
	shift
